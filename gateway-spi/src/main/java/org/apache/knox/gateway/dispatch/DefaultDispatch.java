...
public class DefaultDispatch extends AbstractGatewayDispatch {
  ...
  protected ClassicHttpResponse executeOutboundRequest( ClassicHttpRequest outboundRequest ) throws IOException {
    LOG.dispatchRequest( outboundRequest.getMethod(), outboundRequest.getRequestUri() );
    ClassicHttpResponse inboundResponse;

    try {
      auditor.audit( Action.DISPATCH, outboundRequest.getRequestUri(), ResourceType.URI, ActionOutcome.UNAVAILABLE, RES.requestMethod( outboundRequest.getMethod() ) );
      ...
      auditor.audit( Action.DISPATCH, outboundRequest.getRequestUri(), ResourceType.URI, ActionOutcome.SUCCESS, RES.responseStatus( statusCode ) );
    } catch( Exception e ) {
      // We do not want to expose back end host. port end points to clients, see JIRA KNOX-58
      auditor.audit( Action.DISPATCH, outboundRequest.getRequestUri(), ResourceType.URI, ActionOutcome.FAILURE );
      LOG.dispatchServiceConnectionException( outboundRequest.getRequestUri(), e );
